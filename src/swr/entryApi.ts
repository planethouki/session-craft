import { callGetMyEntries, callGetEntries } from "../firebase";
import type { Entry } from "../models/entry";
import { timestampToDate } from "../utils/dateUtils";

export const getMyEntriesKey = (sessionId: string | undefined) =>
  (sessionId) ? ["getMyEntries", sessionId] : null;
...
export const getEntriesKey = (sessionId: string | undefined) =>
  (sessionId) ? ["getEntries", sessionId] : null;

export const getEntriesFetcher = async ([, sessionId]: [string, string]): Promise<Entry[]> => {
  const res = await callGetEntries({ sessionId });
